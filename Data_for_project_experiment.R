...
doda_efter_region <- as.data.frame(px_data, column.name.type = "text", variable.value.type = "text") 
...
total_död <- aggregate(x = Döda ~ region, data = doda_efter_region, FUN = sum) 

# Convert the region to a factor with levels in the correct order 
total_död$region <- factor(total_död$region, levels = unique(doda_efter_region$region)) 
# Sort by the 'region' factor levels 
total_död <- total_död[order(total_död$region), ] 
